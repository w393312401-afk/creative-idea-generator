...
* **Person-Free Anchors & Worker Entry/Exit Clause**: every IMAGE anchor is a person-free still, so every ordinary construction VIDEO opens on an empty frame, brings the worker in FROM OFF-FRAME immediately after the opening instant straight to the active work face for the first effective tool contact without pausing, and takes the worker FULLY OUT OF FRAME in the closing moment so the last frame matches the empty anchor. The entry and the exit are each one quick move — never a stroll onto the set, never an idle tail. Everything between them is uninterrupted visible work.
  * *Syntax*: `The opening frame is empty of people; immediately after that opening instant the lone worker enters from off-frame straight to the active work face and makes the first effective tool contact without pausing, repeats the visible operation continuously, then steps fully out of frame in the closing moment so the final frame is empty of people again.`
