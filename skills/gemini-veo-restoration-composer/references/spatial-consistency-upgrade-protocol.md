...
graph TD
    ...
    subgraph Agent Locks
        ...
        N[Pre-Bridge Interior Sneak-Peek PBISP] -->|Prevent Threshold Pop-in| K
    end
...
### 7. Clean Frame Boundary & Hero Agent Lock (Clean Frame & HAL) with Out-and-In Clause
...
* **Transient Video Injection**: Workers, tools, and active machinery must exist *only* in `VIDEO 1-N` prompts, entering the frame, performing work, and leaving the frame before the final frame.
...
* **Bi-directional Out-and-In Passage Clause**: To prevent workers from instantly "teleporting" or popping in/out at the start ($t=0s$) and end ($t=8s$) of the time-lapse video clip, every video prompt featuring transient agents must explicitly describe their entry and exit paths.
  * *Syntax*: `At t=0s, the lone worker enters the frame from the far-left Grid C1 edge. The worker performs continuous work at Grid B2, and by t=7.5s, the worker walks out of the frame through the far-left Grid C1 edge, leaving the frame completely empty of agents at the final t=8s frame.`
...
### 8. Pre-Bridge Interior Sneak-Peek with Anchor Inheritance (PBISP)

* **Principle**: Prevent threshold-crossing videos from "exploding" or generating hallucinated interior layouts upon entering a room by establishing spatial continuity beforehand.
