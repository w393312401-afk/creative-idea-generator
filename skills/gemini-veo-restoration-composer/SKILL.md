...
**Post-Crossing Interior Establish Anchor (IMAGE T+1 onward — DLSP 5-Layer Depth & Wide 3/4 Oblique Lock)**:
```
Generate an image of a static 24mm wide-angle interior tripod shot at 1.3m eye-level, wide 3/4 diagonal oblique perspective from the near-corner looking across to the far opposite corner (strict metric envelope: [width]m wide, [depth]m deep, [height]m ceiling clearance). Immediate foreground anchor (<1m): [entrance ladder / portal door jamb] positioned 0.5m in front of the lens on the frame margin at [prose bearing, e.g. "the upper-right ceiling and lower-right foreground"], leaving the center and left frame open. Midground floor (1~4m): [unobstructed broad floor expanse with raw seams and light caustics]. Boundary topology: [Left wall: explicit structure and openings]; [Right wall: explicit solid wall/feature]. Far background wall (>4m): [solid end wall closing depth]. Completely empty of workers, pristine raw trauma state. [Lighting phase] and [material realism]. [Anti-distortion negative restraints].
```
