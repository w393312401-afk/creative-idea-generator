...
1. Never write the full `IMAGE 1-(N+1)` + `VIDEO 1-N` set in your first pass. Compose `IMAGE 1` only, then call `scripts/render_and_gate_anchor.py` **synchronously in the same turn** and wait for its verdict (mechanics in Step 6.5).
2. Only after the gate script exits 0 may the remaining `IMAGE`/`VIDEO` slots be composed and delivered. Exit 0 normally means `auto_approved`; it can also mean `auto_approved_degraded` (judge service errored, frame waved through unverified) — composing may continue, but Step 6.5 rule 4b's disclosure to the user is mandatory. If the script reports the server rewrote the `IMAGE 1` prompt before it passed, the rewritten text it prints is the gated prompt — deliver that verbatim, not your original.
3. Exactly two conditions waive staging and permit the legacy one-pass full-set delivery:
   ...
   - the render server at `http://127.0.0.1:8085` is unreachable (gate script exit code 2) — then state plainly `⚠️ 渲染服务未运行，本次跳过首帧验收，直接输出全套提示词`, deliver in one pass, and still make the Step 13 interactive offer.
4. Ambiguity is **not** a waiver. If unsure whether the user wants images, stage anyway: a gated anchor costs one render; an unverified anchor can waste the entire downstream image and video budget.
...
Staged execution is the **default** per the Staged Delivery Contract (top of this file). Before composing *any* `IMAGE`/`VIDEO` prompt text (Step 7 onward), assume this generation **will** be rendered and follow the staging procedure below. Do not scan the user's wording for render intent ("作图", "渲染", etc.) as a precondition — absence of those words is not a waiver. The only two waivers are: the user explicitly asked for prompt text only (`只要提示词` / `不用作图` / `不用渲染` / `纯文本`), or the render server is unreachable (gate script exit code 2). Under a waiver, deliver the plain prompt set (Steps 7-11 run exactly as written, in one pass) and make the render offer described in Step 13 afterward.

**Unless a waiver applies, everything below is mandatory — not a nice-to-have that only applies to some other execution layer.** Every downstream frame visually chains from `IMAGE 1` (each later frame is generated image-to-image off the previous one), so an unverified anchor silently wastes the entire downstream image and video generation budget if it turns out wrong. This applies exactly the same whether you (the model reading this) are composing the prompts directly in chat, or a separate backend service is driving the composition — do not skip staging just because you are the one writing the prose:

1. Run Steps 1-6 silently as usual (topic parsing through Drift Lock Packet assembly) — nothing user-facing yet.
2. Compose `IMAGE 1`'s prompt only (the relevant slice of Step 7). Do **not** compose `IMAGE 2` onward or any `VIDEO` yet, and do not show `IMAGE 1`'s prompt to the user as a finished deliverable — it is still provisional until it clears the gate below.
3. Call the anchor gate **synchronously, in this same turn**, before writing anything else:
   ```powershell
   ...
   ```
   This call blocks until a real verdict is available (it renders and retries internally, up to 3 attempts, applying the **Anchor Acceptance Gate**: Clean Frame Boundary — zero workers/people/machinery; Camera DNA Plausibility vs. the declared shot type/lens/height/angle; Primary Landmark Presence — the 3 declared anchors visible roughly in their declared depth zones; Genuine Damage — specific construction-grade trauma per the Dirty-State Baseline Vocabulary, not generic/soft; Genre DNA Tone Match — monumental, improbable, visually striking, not mundane; No Text Artifacts). Wait for it to finish; do not proceed speculatively while it's running, and do not fabricate what the verdict "probably" is.
4. **Exit code 0** (`gate_status: auto_approved`): the gate passed. Proceed to step 5 below.
4b. **Exit code 0 with `gate_status: auto_approved_degraded`**: the render server was reachable but its VLM judge errored, so the frame was waved through **unverified** (fail-open). You may continue composing, but you must tell the user plainly that the anchor frame was NOT actually judged (`⚠️ 判定服务异常，首帧为降级放行、未经真实核验`) — never present it as having passed review. Offer to re-run the gate once the judge service recovers.
5. **Exit code 1** (`gate_status: needs_human_review`): the script already retried and rewrote the prompt internally up to 3 times and it still failed. Stop composing immediately — do not silently continue with the failing prompt or invent a passing result. Tell the user plainly that the anchor frame didn't clear automatic review after multiple attempts, show the reported reason, and ask how they'd like to proceed (e.g. adjust the topic, loosen a constraint) rather than delivering an unverified prompt set.
5b. **Exit code 2** (render server unreachable): this is the second waiver in the Staged Delivery Contract. Say `⚠️ 渲染服务未运行，本次跳过首帧验收，直接输出全套提示词`, then fall back to one-pass composition (Steps 7-11 as written) and still make the Step 13 interactive offer. Do not retry the script in a loop.
6. Once the gate passes, treat the confirmed `IMAGE 1` — its final prompt text and what actually rendered — as authoritative reality. Reconcile your working model of the Camera DNA / Primary Landmarks / object ledger against it before writing anything else (this is **Packet Reality Reconciliation**): only rewrite what visibly contradicts the confirmed render; never invent a landmark or object that isn't visible in it.
7. Only now compose `IMAGE 2-(N+1)` (the rest of Step 7) and `VIDEO 1-N` (Step 8), then continue through Steps 9-11 as usual. The final delivered prompt set includes the exact same `IMAGE 1` prompt that already passed the gate, verbatim — do not silently rewrite it again at output time.

This gate is a **hard pass/fail verdict**, not a placeholder awaiting a human's sign-off — a passing anchor is final and composing the rest proceeds autonomously, in the same turn. Step 13's rendering trigger, further down, does not need to re-gate `IMAGE 1` — `/api/render_staged` detects it is already `auto_approved` **and that the `IMAGE 1` prompt is byte-for-byte the one that passed** (it fingerprints the gated prompt), then only renders the remainder. This is one more reason Step 7's rule — deliver the gated `IMAGE 1` prompt verbatim, never silently rewrite it — is load-bearing: an edited `IMAGE 1` prompt voids the stored verdict and forces a re-gate.
...
- **Anchor Acceptance Gate (P0 — staged execution)**: When operating in Staged Execution Mode (Step 6.5), `IMAGE 1` must pass the Anchor Acceptance Gate (Clean Frame Boundary, Camera DNA plausibility, Primary Landmark presence, genuine construction-grade damage, Genre DNA tone match, no text artifacts) before any other `IMAGE`/`VIDEO` is composed or rendered. A failing anchor must be corrected and retried, not silently accepted or left in an unresolved "pending review" state — the gate's verdict is final.
...
2. Compile a structured **Quality Audit & Verification Report** (提示词质量审核报告) in Chinese detailing the status of the generated prompts. The report must contain explicit checks for the SCUP Quality Audit metrics:
   ...
   - **首帧验收门 (Anchor Acceptance Gate — staged execution only)**: When a renderer is driving the skill, the actual rendered `IMAGE 1` — not just its text prompt — is checked against Clean Frame Boundary, Camera DNA plausibility, Primary Landmark presence, genuine construction-grade damage, Genre DNA tone match, and no text artifacts before any other beat is composed or rendered; the packet is then reconciled against the accepted render (Packet Reality Reconciliation) so downstream beats describe confirmed reality, not the pre-visualized spec.
...
This script posts to `/api/render_staged`. If `IMAGE 1` is already `auto_approved` in the project's manifest (the normal case, per Step 6.5), it skips straight to rendering the remainder; otherwise it gates `IMAGE 1` itself first (covering the deferred-decision case above). Either way, every frame/video QA decision afterward is resolved autonomously — there is no "pending human review" dead end in this flow.
...
- Keep the terminal command running until completion so the user can see real-time updates: which frame is generating, any automatic retries, and (in the deferred-decision case) the Anchor Acceptance Gate's verdict on `IMAGE 1`.
