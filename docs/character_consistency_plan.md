...
| **L3 验收层** | 出片后逐帧比对实际画面 | 前两层是否真的生效 | ✅ 剪影级已落地（P2-a）；脸部级（P2-b）未做 |

> 进度（2026-08-15）：**P0 全套 + P2-a 已落地**，L1 从"记得跑 CLI 才有效"变成服务端逐拍
> 自动跑，L3 第一次有了对着真实画面的客观答案。P1（模式 B）与 P2-b（人脸 embedding）未做。
...
**P2-a 剪影级（T1 第 1～8 项，零依赖增量）—— ✅ 已落地 2026-08-15**

实现在 [`cast_drift.py`](../cast_drift.py) + [`tools/check_cast_drift.py`](../tools/check_cast_drift.py)。
落地后与原计划有三处偏差，都是被真实数据逼出来的：

1. **没有做"人物区域粗定位"。** 只有 Pillow + numpy，跑不了人体分割。但也不需要：
   判据改成**同一项目内跨段的相对变化** —— 外套在第 1-6 段稳定占 4% 画面、第 7 段掉到
   0.1%，那就是换装了，无论那 4% 落在画面哪里。
2. **参考色必须人工标定一次。** registry 里存的是词（`faded olive-green work jacket`），
   不是颜色。新增 T1 字段 `reference_srgb`，由 `calibrate` 子命令从一张已认可的帧上取色
   写回。**没有凭空生成颜色** —— 编一个 hex 出来然后让它去当门禁，是最坏的一种假数据。
3. **加了一道原计划没有的防护：掺场景检测。** 拿真实成片实测时，白安全帽的参考色
   同时命中天空，外景段覆盖率 13~20%、近景段 1~2%，于是"基线 1.78%、某段塌到 33%"
   被报成换帽子，真正变的只是构图。现在任一段覆盖率超过 15% 即判该部位不可解释。

三档结论，退出码分开：`0` 有可判部位且都稳定 / `1` 检出漂移 / `2` **判不了**（没标定、
或所有部位都不可用）。`2` 单独一个码就是为了让"什么都没测出来"没法被当成"通过"混过
交付门 —— 那和它要取代的人眼放水是同一种错误，只是换了个更权威的口气。

已知盲区（报告里逐部位给出基线与峰值覆盖率，就是让人能自己判断可信度）：
换成相近色（ΔE 落在容差内）抓不到；人物太小则任何部位都达不到最低覆盖率；
容差 12 下余量最窄的一对是橄榄绿↔军绿（ΔE 14.5），强烈光影切换有可能漏报。
