...
### 4.1 两个独立的密度旋钮（2026-08-11 补）

用户反馈「识别的图片太少」。这句话对应的其实是两处上限，必须分开调，否则调了
一处另一处照样卡着：

- **抽帧密度**（`--base-fps`，`replica_pipeline.EXTRACT_FPS_CHOICES`）——一共抽出
  多少帧可选。默认 2fps 是为延时片调的，慢工序（刮腻子、铺砖）在两张之间就跨过
  半个工序。抽帧是本地 ffmpeg，不花模型钱，可以放心往上调，代价只是更慢。
  换密度**必须重抽并清掉帧事实缓存**：帧文件名是序号不是时间戳，同一个名字在新
  密度下指向另一时刻的画面，留着缓存就是静默错位。
- **送审档位**（`reverse.REVIEW_SCOPES`：`degraded` / `plan` / `all`）——抽出来的
  帧里送多少张给多模态模型。`analysis_plan` 是硬**下界**不是上界：长片走 adaptive
  时它只挑约四成，`all` 档存在的意义就是把这个上界打开。这一档直接决定账单。

反推段的模型选择（`frameFactsModel` 逐帧识别 / `peakVerifyModel` 峰值复核，后者
写 `off` 可关）同期摆到了成本确认卡点上——此前只能改配置文件，而页面上的「LLM 模型」
选择器管的是激发/合成那条链路，改它对反推毫无影响。托管模式下这两个键必须在
`server_common._PASSTHROUGH_CLIENT_KEYS` 里，否则就是「选了但从未生效」。
