...
| **P3** ✅ 已完成 | §F 工具条：筛选 + 多选 + 尺寸档 + 一键全部修复 + 撤销修复 | 新增 | `tests/test_slot_toolbar.py`（33 项）全绿 |
...
* **一键全部修复**（图片网格专有）：把所有画着「修复此帧问题」的格子——
  一致性审查未过 + 人工标记，判定收口在 `slot_model.frameIsFixable`，卡片按钮
  与工具条读同一条——按帧号**升序串行**走一遍 `fixFrameIssue`。审查一次能标出
  七八帧，此前只能一格一格点、还得盯着每一次复核结论。升序是因为非首帧走
  图生图链式编辑：修完 IMG 003 再修 IMG 005 时，后者读到的已经是新的 IMG 004。
  每修完一帧回读一次 manifest（复核结论只写在服务端 manifest 上，事件流里没有
  对应的 `frame` 事件），轮到某帧时它已无待修问题就跳过。`fixFrameIssue` 为此
  改成返回 `{status}`：取消/失败会中止整轮，而不是接着修下一帧。
* **撤销修复**（2026-08-02）：定向修复是覆盖写同一个帧文件，旧图此前不留档——
  修坏了只能盲重渲碰运气。现在每次修复动手前存一份快照（帧图 + 该帧的 manifest
  条目 + 这一帧与前一拍视频的提示词正文，落在 `.frame_fixes/<seq>/`，只留最近
  一次），卡片上随之出现「撤销修复」。回滚**只换回这一帧涉及的两个槽位**，不整体
  还原 `prompt_block`——修完 003 又修了 005 之后撤销 003，整体还原会把 005 的修复
  一起吞掉。撤销与修复共用同一个收尾（`update_manifest_stale_status(finalize=True)`）：
  画面又变了一次，下游血统、相邻拍的审查结论、已合并成片都要跟着作废。
